...
def upload_file(file_path):
    ...
    mud_water_header_row = find_section_start(df, 'Properties - water|Properties - Water|Mud Water', column=0)
    if mud_water_header_row is None:
        ...
        water_start = mud_water_header_row + 1
        water_nrows = 0
        next_section_start = find_section_start(df, 'Properties - oil|Properties -oil|Oil based mud', column=0)
        for i in range(water_start, len(df) if next_section_start is None else next_section_start):
            val = df.iloc[i, 0]
            if pd.isna(val):
                continue
            cleaned = str(clean_value(val)).strip()
            ...
            water_nrows += 1
        try:
            mud_water_df = pd.read_excel(file_path, sheet_name='Sheet1', skiprows=mud_water_header_row + 1, nrows=water_nrows + 2, header=0, usecols="A:Z")
            mud_water_df.columns = [col.strip().replace('_x000D_', '').replace('\n', ' ').replace('\r', '').replace('  ', ' ').replace(' (', '_(').replace(' )', ')').replace('°F', '_f').replace('%', '_pct').replace('lb/bbl', '_lb_bbl').replace('(mg/L)', '_mg_l').replace('(1/32in)', '_1_32in').strip() for col in mud_water_df.columns]
            rename_map = {'Rpt No.': 'rpt_no', 'Date': 'date', 'MD_(ft)': 'md_ft', 'Sample from': 'sample_from', 'MW_(ppg)': 'mw_ppg', 'Funnel visc. (sec/qt)': 'funnel_visc_sec_qt', 'PV_(cP)': 'pv_cp', 'YP_(lbf/100ft2)': 'yp_lbf_100ft2', '6 RPM': '_6_rpm', 'API filtrate (ml/30min)': 'api_filtrate_ml_30min', 'API cake thickness (1/32in)': 'api_cake_thickness_1_32in', 'LGS (%)': 'lgs_pct', 'Solids (%)': 'solids_pct', 'Oil (%)': 'oil_pct', 'Water (%)': 'water_pct', 'Sand content (%)': 'sand_content_pct', 'MBT capacity (lb/bbl)': 'mbt_capacity_lb_bbl', 'pH': 'ph', 'Mud alkalinity (Pm) (ml)': 'mud_alkalinity_pm_ml', 'Filtrate alkalinity (Pf) (ml)': 'filtrate_alkalinity_pf_ml', 'Filtrate alkalinity (Mf) (ml)': 'filtrate_alkalinity_mf_ml', 'Calcium (mg/L)': 'calcium_mg_l', 'Chlorides (mg/L)': 'chlorides_mg_l', 'Total hardness (mg/L)': 'total_hardness_mg_l', 'Excess lime (lb/bbl)': 'excess_lime_lb_bbl', 'Fine LCM (lb/bbl)': 'fine_lcm_lb_bbl', 'Coarse LCM (lb/bbl)': 'coarse_lcm_lb_bbl'}
            ...
            for _, row in mud_water_df.iterrows():
                ...
                if rpt is None or not str(rpt).strip().replace('.', '').isdigit():
                    continue
                water_data = {"well_id": well_id, "rpt_no": int(float(rpt)), "date": clean_value(row.get('date')), "md_ft": safe_float(row.get('md_ft')), "sample_from": clean_value(row.get('sample_from')), "mw_ppg": safe_float(row.get('mw_ppg')), "funnel_visc_sec_qt": safe_float(row.get('funnel_visc_sec_qt')), "pv_cp": safe_float(row.get('pv_cp')), "yp_lbf_100ft2": safe_float(row.get('yp_lbf_100ft2')), "_6_rpm": safe_float(row.get('_6_rpm')), "api_filtrate_ml_30min": safe_float(row.get('api_filtrate_ml_30min')), "api_cake_thickness_1_32in": safe_float(row.get('api_cake_thickness_1_32in')), "lgs_pct": safe_float(row.get('lgs_pct')), "solids_pct": safe_float(row.get('solids_pct')), "oil_pct": safe_float(row.get('oil_pct')), "water_pct": safe_float(row.get('water_pct')), "sand_content_pct": safe_float(row.get('sand_content_pct')), "mbt_capacity_lb_bbl": safe_float(row.get('mbt_capacity_lb_bbl')), "ph": safe_float(row.get('ph')), "mud_alkalinity_pm_ml": safe_float(row.get('mud_alkalinity_pm_ml')), "filtrate_alkalinity_pf_ml": safe_float(row.get('filtrate_alkalinity_pf_ml')), "filtrate_alkalinity_mf_ml": safe_float(row.get('filtrate_alkalinity_mf_ml')), "calcium_mg_l": safe_float(row.get('calcium_mg_l')), "chlorides_mg_l": safe_float(row.get('chlorides_mg_l')), "total_hardness_mg_l": safe_float(row.get('total_hardness_mg_l')), "excess_lime_lb_bbl": safe_float(row.get('excess_lime_lb_bbl')), "fine_lcm_lb_bbl": safe_float(row.get('fine_lcm_lb_bbl')), "coarse_lcm_lb_bbl": safe_float(row.get('coarse_lcm_lb_bbl'))}
                try:
                    insert_row('MudPropertiesWater', water_data)
                    inserted_w += 1
                except Exception as e:
                    print(f"  Mud Water insert failed for rpt_no {rpt}: {e}")
            print(f"→ Inserted {inserted_w} mud properties water rows")
        except Exception as e:
            print(f"Failed to read or insert Mud Properties Water: {e}")
            
    # ── Mud Properties Oil ────────────────────────────────────────────────
    oil_header = find_section_start(df, 'Properties - oil|Properties -oil|Oil based mud', column=0)
    if oil_header is None:
        ...
        oil_nrows = 0
        next_section_start = find_section_start(df, 'Rheology|Rheo|Gel Strength', column=0)
        for i in range(oil_start, len(df) if next_section_start is None else next_section_start):
            val = df.iloc[i, 0]
            if pd.isna(val):
                continue
            cleaned = str(clean_value(val)).strip()
            ...
            oil_nrows += 1
        try:
            oil_df = pd.read_excel(file_path, sheet_name='Sheet1', skiprows=oil_header + 1, nrows=oil_nrows + 2, header=0, usecols="A:Z")
            oil_df.columns = [col.strip().replace('_x000D_', '').replace('\n', ' ').replace('\r', '').replace('  ', ' ').replace(' (', '_(').replace(' )', ')').replace('°F', '_f').replace('%', '_pct').replace('lb/bbl', '_lb_bbl').replace('(mg/L)', '_mg_l').replace('(1/32in)', '_1_32in').strip() for col in oil_df.columns]
            rename_map = {'Rpt No.': 'rpt_no', 'Date': 'date', 'MD_(ft)': 'md_ft', 'Sample from': 'sample_from', 'MW_(ppg)': 'mw_ppg', 'Funnel visc. (sec/qt)': 'funnel_visc_sec_qt', 'PV_(cP)': 'pv_cp', 'YP_(lbf/100ft2)': 'yp_lbf_100ft2', '6 RPM': '_6_rpm', 'HTHP filtrate (ml/30min)': 'hthp_filtrate_ml_30min', 'HTHP cake thickness (1/32in)': 'hthp_cake_thickness_1_32in', 'LGS (%)': 'lgs_pct', 'Solids (%)': 'solids_pct', 'Oil (%)': 'oil_pct', 'Water (%)': 'water_pct', 'Oil/water ratio': 'oil_water_ratio', 'Alkalinity mud (pom) (cc/cc)': 'alkalinity_cc', 'Excess lime (lb/bbl)': 'excess_lime_lb_bbl', 'Chlorides whole mud (mg/L)': 'chlorides_mg_l', 'Solids adjusted for salt (%)': 'solids_salt_pct', 'Salt content water phase (%)': 'salt_phase_pct', 'WPS (ppm)': 'wps', 'Whole mud Ca (CAom) (mg/L)': 'whole_caom_mg_l', 'Electrical stability(Volt)': 'electrical_stability_volt', 'Water activity(Aw)': 'water_activity_aw', 'Fine LCM (lb/bbl)': 'fine_lcm_lb_bbl', 'Coarse LCM (lb/bbl)': 'coarse_lcm_lb_bbl'}
            ...
            print(f"Failed to read or insert Mud Properties Oil: {e}")
    ...
    if rheo_header is None:
        ...
        rheo_nrows = 0
        next_section_start = find_section_start(df, 'Solids analysis|Solids Analysis|Solids', column=0)
        for i in range(rheo_start, len(df) if next_section_start is None else next_section_start):
            val = df.iloc[i, 0]
            if pd.isna(val):
                continue
            cleaned = str(clean_value(val)).strip()
        ...
        try:
            ...
            # (column cleaning and rename_map stay exactly as before)
            rheo_df.columns = [col.strip().replace('_x000D_', '').replace('\n', ' ').replace('\r', '').replace('  ', ' ').replace(' (', '_(').replace(' )', ')').replace('°F', '_f').replace('RPM', '_rpm').replace('Gel Str.', 'gel').replace('10 sec', '_10sec').replace('10 min', '_10min').replace('30 min', '_30min').replace('lbf/100ft2', 'lbf_100ft2').strip() for col in rheo_df.columns]
    ...
    if solids_header is None:
        ...
        solids_nrows = 0
        next_section_start = find_section_start(df, 'Volumes|Volume|Daily addition|Start vol.|End vol.', column=0)
        for i in range(solids_start, len(df) if next_section_start is None else next_section_start):
            val = df.iloc[i, 0]
            if pd.isna(val):
                continue
            cleaned = str(clean_value(val)).strip()
        ...
        try:
            ...
            solids_df.columns = [col.strip().replace('_x000D_', '').replace('\n', ' ').replace('\r', '').replace('  ', ' ').replace(' (', '_(').replace(' )', ')').replace('lb/bbl', '_lb_bbl').replace('%', '_pct').strip() for col in solids_df.columns]
            rename_map = {'Rpt No.': 'rpt_no', 'Date': 'date', 'MD_(ft)': 'md_ft', 'LGS_(_pct)': 'lgs_pct', 'LGS_(_lb_bbl)': 'lgs_lb_bbl', 'HGS_(_pct)': 'hgs_pct', 'HGS_(_lb_bbl)': 'hgs_lb_bbl', 'Bentonite_(_pct)': 'bentonite_pct', 'Bentonite_(_lb_bbl)': 'bentonite_lb_bbl', 'Drill solids_(_pct)': 'drill_solids_pct', 'Drill solids_(_lb_bbl)': 'drill_solids_lb_bbl', 'DS/Bent ratio': 'ds_bent_ratio', 'OBM chemicals_(_pct)': 'obm_pct', 'OBM chemicals_(_lb_bbl)': 'obm_lb_bbl', 'Avg. SG of solids': 'avg_sg_solids'}
    ...
    if volumes_header is None:
        ...
        volumes_nrows = 0
        next_section_start = find_section_start(df, 'Cost|Cost table|Daily cost|Cum. cost', column=0)
        for i in range(volumes_start, len(df) if next_section_start is None else next_section_start):
            val = df.iloc[i, 0]
            if pd.isna(val):
                continue
            cleaned = str(clean_value(val)).strip()
        ...
        try:
            ...
            volumes_df.columns = [col.strip().replace('_x000D_', '').replace('\n', ' ').replace('\r', '').replace('  ', ' ').replace(' (', '_(').replace(' )', ')').replace('bbl', '_bbl').replace('%', '_pct').strip() for col in volumes_df.columns]
            rename_map = {'Rpt No.': 'rpt_no', 'Date': 'date', 'MD_(ft)': 'md_ft', 'Start vol._(bbl)': 'start_vol_bbl', 'Other mud_(bbl)': 'other_mud_bbl', 'Base fluid_(bbl)': 'base_mud_bbl', 'Water_(bbl)': 'water_bbl', 'Products_(bbl)': 'products_bbl', 'Weight materials_(bbl)': 'weight_materials_bbl', 'Formation_(bbl)': 'formation_bbl', 'Cuttings_(bbl)': 'cuttings_bbl', 'Others_(bbl)': 'others_bbl', 'Daily addition_(bbl)': 'daily_addition_bbl', 'Cuttings/retention_(bbl)': 'cuttings_retention_bbl', 'Seepage_(bbl)': 'seepage_bbl', 'Dump_(bbl)': 'dump_bbl', 'Shakers_(bbl)': 'shakers_bbl', 'Centrifuge_(bbl)': 'centrifuge_bbl', 'Evaporation_(bbl)': 'evaporation_bbl', 'Pit cleaning_(bbl)': 'pit_cleaning_bbl', 'Formation loss_(bbl)': 'formation_loss_bbl', 'Others_(bbl).1': 'others_loss_bbl', 'Daily loss_(bbl)': 'daily_loss_bbl', 'From storage_(bbl)': 'from_storage_bbl', 'To storage_(bbl)': 'to_storage_bbl', 'End vol._(bbl)': 'end_vol_bbl', 'Total on location_(bbl)': 'on_location_bbl'}
    ...
    if cost_header is None:
        ...
        cost_nrows = 0
        next_section_start = find_section_start(df, 'Remarks|Remarks Section|Notes|Comments', column=0)
        for i in range(cost_start, len(df) if next_section_start is None else next_section_start):
            val = df.iloc[i, 0]
            if pd.isna(val):
                continue
            cleaned = str(clean_value(val)).strip()
        ...
        try:
            ...
            cost_df.columns = [col.strip().replace('_x000D_', '').replace('\n', ' ').replace('\r', '').replace('  ', ' ').replace(' (', '_(').replace(' )', ')').replace('($)', '_s').replace('(s)', '_s').replace('Cum.', 'cum').replace('Daily', 'daily').strip() for col in cost_df.columns]
            rename_map = {'Rpt No.': 'rpt_no', 'Date': 'date', 'MD_(ft)': 'md_ft', 'Product_(s)': 'product_s', 'Package_(s)': 'package_s', 'Service_(s)': 'service_s', 'Engineering_(s)': 'engineering_s', 'Tax_(s)': 'tax_s', 'Daily total_(s)': 'daily_s', 'Cum. cost_(s)': 'cum_s', 'Total cost': 'total_cost'}
    ...
    if remarks_header is None:
        ...
        remarks_nrows = 0
        # No next section — go to end of sheet
